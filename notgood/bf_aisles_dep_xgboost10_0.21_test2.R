...
prd <- prd %>%
  inner_join(products[, c(1, 4:7)], by = "product_id")
...
users <- orders %>%
  ...
  summarise(
    ...
    user_days_mean_since_prior_order30 = mean(days_since_prior_order == 30, na.rm = T),
    user_days_sd_since_prior_order30 = sd(days_since_prior_order == 30, na.rm = T)
  )
...
users$user_prior_period_ratio <- users$user_mean_days_since_prior / users$user_period
users$user_priorsd_period_ratio <- users$user_sd_days_since_prior / users$user_period
...
test$reordered <- ((out3+ out4 + out6)/3)
...
write.csv(submission, file = "bf_xgboost_0.211_sq0.3_test.csv", row.names = F)
