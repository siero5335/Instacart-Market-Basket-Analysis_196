...
products_tsnes <- fread("tsnes_productname.csv")
...
res_pca_prod <- prcomp(products_tsnes[,2:101])


set.seed(71)
res_tsne_prod <- Rtsne(as.matrix(res_pca_prod$x[,1:40]), dims = 2, initial_dims = 40, perplexity = 5,
                       theta = 0.7, check_duplicates = FALSE, pca = TRUE, max_iter = 500,
                       verbose = TRUE)


prod_vec_tsne <- data.frame(products_tsnes[,1], res_tsne_prod$Y[, 1:2])


products <- products %>% 
  left_join(prod_vec_tsne)

rm(prod_vec_pca, products_tsnes, prod_vec_tsne, res_pca_prod)
gc()
...
set.seed(71)
test1 <- Rtsne(as.matrix(users[, -c(23, 25, 28)]), dims = 2, initial_dims = 50, perplexity = 30,
...
#model1
set.seed(72)
subtrain1 <- train %>% sample_frac(0.5)
X1 <- lgb.Dataset(as.matrix(subtrain1 %>% select(-reordered)), label = subtrain1$reordered)

num_iterations <- 3000 #search
set.seed(71)
bst.cv <- lgb.cv(params = params, data = X1, nfold = 4, nrounds=num_iterations, early_stopping_rounds= 5)
...
lgb.plot.importance(tree_imp, top_n = 31, measure = "Gain")
rm(X1, tree_imp)
gc()
