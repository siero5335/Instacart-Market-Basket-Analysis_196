...
set.seed(71)
model4 <- xgboost(data = X1, params = params, nrounds = nround)
...
set.seed(71)
model5 <- xgboost(data = X1, params = params, nrounds = nround)
...
set.seed(71)
model6 <- xgboost(data = X1, params = params, nrounds = nround)
...
test$reordered <- (out1 + out2 + out3 + out4 + out5 + out6)/6

write.csv(test, "test_bf_tsne_aisles_dep_6xgboost_F1max.csv")
