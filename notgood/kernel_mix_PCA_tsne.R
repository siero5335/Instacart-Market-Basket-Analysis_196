...
aisles <- tmp %>% left_join(aisles,by="aisle_id")
...
data$organic <- as.numeric(data$organic)
...
data2 <- prcomp(data[, c(3:7, 9, 11: 20, 23:26)], scale = TRUE)
...
data[, c(3:7, 9, 11: 20, 23:26)] <- scale(data[, c(3:7, 9, 11: 20, 23:26)])

data$mix_order <- scale((data$user_average_basket)*(data$organic)*(data$user_id))
data$mix_order <- data$mix_order[,1]
...
write.csv(submission, file = "mix_order_marge3xgboost_0.16_sq_77.csv", row.names = F)
