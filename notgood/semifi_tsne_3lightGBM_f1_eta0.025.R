...
train$prod_median_add_to_cart_inverted <- NULL
train$up_orders <- NULL
train$user_mean_orders <- NULL
...
test$prod_median_add_to_cart_inverted <- NULL
test$up_orders <- NULL
test$user_mean_orders <- NULL
...
#model1
set.seed(114514)
train_part <- createDataPartition(train$reordered, p=0.75, list=FALSE)
...
lgb.plot.importance(tree_imp, top_n = 124, measure = "Gain")
