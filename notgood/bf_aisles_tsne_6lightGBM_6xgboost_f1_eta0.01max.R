...
prd <- prd %>%
  inner_join(products[, c(1, 3:10)], by = "product_id")
...
set.seed(71)
test1 <- Rtsne(as.matrix(prd[, -c(5:13, 29)]), dims = 2, initial_dims = 50, perplexity = 30,
...
set.seed(71)
test1 <- Rtsne(as.matrix(users[, -c(18, 23, 25)]), dims = 2, initial_dims = 50, perplexity = 30,
...
rm(prd, users, ord2, products, bf, orders)
...
#model1
set.seed(72)
subtrain1 <- train %>% sample_frac(0.6)
X1 <- lgb.Dataset(as.matrix(subtrain1 %>% select(-reordered)), label = subtrain1$reordered)

num_iterations <- 1000 #search
set.seed(71)
bst.cv <- lgb.cv(params = params, data = X1, nfold = 4, nrounds=num_iterations, early_stopping_rounds= 3)
nround <- bst.cv$best_iter

set.seed(71)
model1_1 <- lgb.train(data = X1, params = params, nrounds = nround)
...
set.seed(71)
bst.cv <- lgb.cv(params = params, data = X1, nfold = 4, nrounds=cv.nround, early_stopping_rounds= 3)
nround <- bst.cv$best_iter

set.seed(71)
model2_1 <- lgb.train(data = X1, params = params, nrounds = nround)
...
set.seed(71)
model3_1 <- lgb.train(data = X1, params = params, nrounds = nround)
...
set.seed(71)
model4_1 <- lgb.train(data = X1, params = params, nrounds = nround)
...
set.seed(71)
model5_1 <- lgb.train(data = X1, params = params, nrounds = nround)
...
set.seed(71)
model6_1 <- lgb.train(data = X1, params = params, nrounds = nround)
...
test$reordered <- (out1_1 + out2_1 + out3_1 + out4_1 + out5_1 + out6_1+ out1 + out2 + out3 + out4 + out5 + out6)/12

write.csv(test, "test_bf_tsne_aisles_dep_6lightboost_6xgboost_F1max")