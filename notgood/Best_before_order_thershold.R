...
prd <- prd %>%
  inner_join(products[, c(1, 3:7)], by = "product_id")
...
data <- data %>%
  left_join(streaks,  by = c("user_id", "product_id"))

data <- data %>% 
  left_join(ordert %>% select(user_id, product_id, reordered), 
            by = c("user_id", "product_id"))

rm(ordert, prd, users, ord2, products, streaks)
gc()
...
write.csv(test, "predicted_testset.csv")

test$reordered <- (test$reordered > 0.211) * 1
...
write.csv(submission, file = "bf_xgboost_0.211_sq0.3_5.csv", row.names = F)




X2 <- xgb.DMatrix(as.matrix(train %>% select(-reordered)))
out1 <- predict(model1, X2)
out2 <- predict(model2, X2)
out3 <- predict(model3, X2)
out4 <- predict(model4, X2)
out5 <- predict(model5, X2)
out6 <- predict(model6, X2)
out7 <- predict(model7, X2)
out8 <- predict(model8, X2)
out9 <- predict(model9, X2)

train$reordered2 <- ((out1 + out2 + out3+ out4 + out5 + out6 + out7 + out8 + out9)/9)
write.csv(train, "predicted_trainset.csv")