...
prd <- orders_products %>%
  ...
  summarise(
    ...
    prod_days_since_prior_order7 = sum(days_since_prior_order == 7, na.rm = T),
    prod_days_since_prior_order14 = sum(days_since_prior_order == 14, na.rm = T),
    prod_days_since_prior_order30 = sum(days_since_prior_order == 30, na.rm = T)
  )
...
prd <- prd %>%
  inner_join(products[, c(1, 3, 6:8)], by = "product_id")
...
temp <- prd2 %>%
  group_by(order_id) %>%
  summarise(
    ord_mean_cart = mean(user_cart),
    ord_sd_cart = sd(user_cart)
  )

ord2 <- prd %>% inner_join(temp)
...
rm(ordert, prd, users, ord2, products)
gc()
...
cv.nround <- 10000 #search
set.seed(71)
bst.cv <- xgb.cv(params = params, data = X2, nfold = 4, nrounds=cv.nround, early_stopping_rounds=2)
nround <- bst.cv$best_iteration
...
cv.nround <- 10000 #search
set.seed(71)
bst.cv <- xgb.cv(params = params, data = X3, nfold = 4, nrounds=cv.nround, early_stopping_rounds=2)
nround <- bst.cv$best_iteration
...
cv.nround <- 10000 #search
set.seed(71)
bst.cv <- xgb.cv(params = params, data = X4, nfold = 4, nrounds=cv.nround, early_stopping_rounds=2)
nround <- bst.cv$best_iteration
...
cv.nround <- 10000 #search
set.seed(71)
bst.cv <- xgb.cv(params = params, data = X5, nfold = 4, nrounds=cv.nround, early_stopping_rounds=2)
nround <- bst.cv$best_iteration
...
cv.nround <- 10000 #search
set.seed(71)
bst.cv <- xgb.cv(params = params, data = X6, nfold = 4, nrounds=cv.nround, early_stopping_rounds=2)
nround <- bst.cv$best_iteration
...
cv.nround <- 10000 #search
set.seed(71)
bst.cv <- xgb.cv(params = params, data = X7, nfold = 4, nrounds=cv.nround, early_stopping_rounds=2)
nround <- bst.cv$best_iteration
...
cv.nround <- 10000 #search
set.seed(71)
bst.cv <- xgb.cv(params = params, data = X8, nfold = 4, nrounds=cv.nround, early_stopping_rounds=2)
nround <- bst.cv$best_iteration
...
cv.nround <- 10000 #search
set.seed(71)
bst.cv <- xgb.cv(params = params, data = X9, nfold = 4, nrounds=cv.nround, early_stopping_rounds=2)
nround <- bst.cv$best_iteration
...
write.csv(submission, file = "10xgboost_0.209_sq0.3_eta0.05.csv", row.names = F)