...
xgb_eval_f1 <- function (yhat, train) {
...
X1 <- xgb.DMatrix(as.matrix((train[!user_id %in% val_users]), 
                            label=train[!user_id %in% val_users, reordered]))
...
write.csv(submission, file = "submit_kernel_marge3xgboost_0.25.csv", row.names = F)
