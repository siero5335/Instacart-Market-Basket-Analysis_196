...
prd <- orders_products %>%
  ...
  summarise(
    prod_orders = n(),
    prod_hour = mean(order_hour_of_day),
...
prd$cart_reoder_ratio <-  prd$prod_reorders / prd$prod_cart
...
temp <-  
  ...
  summarise(
  dep_ave_reorder_probability = mean(prod_reorder_probability)
  ) %>%
  mutate(dep_ave_reorder_probability)

prd <- prd %>%
  inner_join(temp, by = "department_id")

temp <-  
  prd %>%
  group_by(aisle_id) %>%
  summarise(
    aisle_ave_reorder_probability = mean(prod_reorder_probability)
  ) %>%
  mutate(aisle_ave_reorder_probability)

prd <- prd %>%
  inner_join(temp, by = "aisle_id")

temp <-  
  prd %>%
  group_by(organic) %>%
  summarise(
    org_ave_reorder_probability = mean(prod_reorder_probability)
  ) %>%
  mutate(org_ave_reorder_probability)

prd <- prd %>%
  inner_join(temp, by = "organic")
...
data$up_order_rate <- (data$up_orders / data$user_orders)^2
data$up_orders_since_last_order <- (data$user_orders - data$up_last_order)^2
data$up_order_rate_since_first_order <- ((data$up_orders) / (data$user_orders - data$up_first_order + 1))^2
...
write.csv(submission, file = "kernel_feature2_mod_marge3xgboost_0.21_.csv", row.names = F)
