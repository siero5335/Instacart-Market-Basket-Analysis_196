...
train2 <- train[, c(1, 6, 7, 8, 13, 16:19)]
train2 <- as.matrix(train2)

X <- xgb.DMatrix(train2, label = train$reordered)
...
rm(X, importance, train)
gc()


# Apply model -------------------------------------------------------------
test2 <- test[, c(2, 7, 8, 9, 14, 17:20)]
test2 <- as.matrix(test2)


X <- xgb.DMatrix(as.matrix(test2))
...
write.csv(submission, file = "submit_kernel_selected2.csv", row.names = F)