...
data$up_order_prod_reorder_probability2 <- (data$prod_reorder_probability)^2
...
data2 <- prcomp(data[, c(3:7, 9, 11: 20, 23:30)], scale = TRUE)
...
library(Rtsne)
data[, c(3:7, 9, 11: 20, 23:30)] <- scale(data[, c(3:7, 9, 11: 20, 23:30)])


data3 <- Rtsne(data[, c(3:7, 9, 11: 20, 23:30)], theta=0.3, max_iter=100)
...
write.csv(submission, file = "kernel_mix_marge3xgboost_0.21_sq_77.csv", row.names = F)